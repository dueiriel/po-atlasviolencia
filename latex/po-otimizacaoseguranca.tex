...
\begin{titlepage}
    ...
    \begin{minipage}{8cm}
        \singlespacing
        \small
        Prof. Eduardo Noronha de Andrade Freitas
    \end{minipage}
...
\noindent
Este trabalho aplica técnicas de Programação Linear para otimizar a alocação de recursos de segurança pública entre os 27 estados brasileiros. Utilizando dados reais do Anuário Brasileiro de Segurança Pública (FBSP) no período de 2013-2023, desenvolvemos um modelo matemático que determina a distribuição ótima de um orçamento suplementar visando minimizar o número de mortes violentas. Os resultados indicam que, com um investimento adicional de R\$ 5 bilhões distribuído de forma otimizada, seria possível salvar aproximadamente 1.875 vidas por ano. Além disso, identificamos que estados como São Paulo, Santa Catarina e Distrito Federal apresentam maior eficiência no uso de recursos. O trabalho inclui simulação Monte Carlo para validação dos resultados.\\[0.5cm]
...
\midrule
Anuário FBSP & Mortes violentas intencionais por UF & 2013-2023 & forumseguranca.org.br \\
SICONFI & Gastos com Segurança Pública & 2013-2023 & siconfi.tesouro.gov.br \\
\bottomrule
\end{tabularx}
...
Os dados de mortes violentas intencionais (MVI) seguem a metodologia do Fórum Brasileiro de Segurança Pública, que inclui: homicídios dolosos, latrocínios (roubo seguido de morte), lesões corporais seguidas de morte e mortes decorrentes de intervenção policial. A fonte primária é o Anuário Brasileiro de Segurança Pública (FBSP, 2023), com dados disponíveis para o período 2013-2023.
...
\begin{figure}[H]
    ...
    \caption{Taxa de mortes violentas por 100 mil habitantes (2023)}
    \label{fig:ranking}
    \footnotesize{Fonte: Elaboração própria com dados do Anuário FBSP (2026).}
\end{figure}
...
\begin{figure}[H]
    ...
    \footnotesize{Fonte: Elaboração própria com dados do Anuário FBSP e SICONFI (2026).}
\end{figure}
...
Para medir a eficiência relativa de cada estado, utilizamos uma abordagem inspirada em DEA (Data Envelopment Analysis) com pesos fixos, priorizando o resultado (baixa taxa de homicídios) sobre a economia (baixo gasto):

\begin{equation}
\text{Eficiência}_i = 0{,}75 \cdot \frac{\text{Taxa}_{\min}}{\text{Taxa}_i} + 0{,}25 \cdot \frac{\text{Gasto}_{\min}}{\text{Gasto}_i}
\end{equation}

Onde:
\begin{itemize}
    \setlength{\itemsep}{0pt}
    \item \textbf{75\%} do peso é dado ao \textbf{resultado} (quanto menor a taxa de homicídios, melhor)
    \item \textbf{25\%} do peso é dado à \textbf{economia} (quanto menor o gasto per capita, melhor)
\end{itemize}

Esta ponderação garante que estados com baixa violência sejam priorizados no ranking, mesmo que gastem mais que outros. A eficiência é normalizada para que o estado mais eficiente tenha 100\%.
...
Os cinco estados mais eficientes são: São Paulo (100\%), Santa Catarina (79,2\%), Distrito Federal (76,3\%), Minas Gerais (48,8\%) e Piauí (48,8\%). Estes estados conseguem as menores taxas de violência do país, sendo São Paulo o benchmark com a menor taxa (7,46 por 100 mil).

Os cinco menos eficientes são: Amapá (16,9\%), Mato Grosso (25,3\%), Roraima (28,1\%), Amazonas (29,6\%) e Alagoas (30,1\%). Estes estados combinam altas taxas de violência (acima de 27 por 100 mil) com gastos elevados por habitante.

\subsection{Resultado da Otimização}

Executamos o modelo com orçamento suplementar de R\$ 5 bilhões.
...
A Figura 4 mostra quanto cada estado deve receber segundo o modelo.
...
Os estados que mais recebem são aqueles com alto número absoluto de mortes (maior potencial de impacto) e margem para aumento (orçamento atual não está no limite).
...
Primeiro, a eficiência no uso de recursos de segurança varia significativamente entre os estados brasileiros. Utilizando a fórmula de eficiência DEA com pesos (75\% resultado + 25\% economia), estados como São Paulo e Santa Catarina lideram o ranking, conseguindo baixas taxas de violência com gastos moderados.

Segundo, a otimização gera impacto significativo. Uma distribuição otimizada de R\$ 5 bilhões poderia salvar aproximadamente 1.875 vidas por ano, com intervalo de confiança de 95\% entre 1.604 e 2.452 vidas (validado por simulação Monte Carlo).

Terceiro, gasto não garante resultado. Estados do Norte gastam muito acima da média nacional, mas mantêm taxas de violência elevadas. A qualidade da gestão é determinante.

Quarto, a Programação Linear demonstra ser uma ferramenta eficaz para apoio à decisão em políticas públicas, permitindo identificar alocações ótimas de recursos escassos de forma objetiva e transparente.
...
\noindent
SICONFI. \textbf{Sistema de Informações Contábeis e Fiscais do Setor Público Brasileiro}. Brasília: Tesouro Nacional, 2023. Disponível em: https://siconfi.tesouro.gov.br/. Acesso em: 30 jan. 2026.\\[0.3cm]
...
\footnotesize{Fonte: Anuário FBSP e SICONFI 2023.}
\end{table}
...
\begin{center}
\url{https://github.com/dueiriel/po-otimizacaoviolencia}
\end{center}

A aplicação web interativa pode ser acessada em:

\begin{center}
\url{http://po-otimizacaoseguranca.duckdns.org}
\end{center}
